...
public:
	ammo() : item() {}
	ammo(float _caliber, float _ap, item item_data)
...
public:
	void set_caliber(float val) { caliber = val; }
	float get_caliber() const { return caliber; }
...
float total_weight(vector<item>& items)
{
	...
	for (vector<item>::iterator iter = items.begin(); iter != items.end(); iter++)
		weight += iter->get_weight() * iter->get_count();
	return weight;
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	cin >> wnum >> anum >> armour >> seconds;
	cout << "\nEach hit will deal " << weapons[wnum].damage_dealt(0, armour, &ammos[anum])
		<< " damage.\nTotal damage is " << weapons[wnum].damage_dealt(seconds, armour, &ammos[anum]) << endl;
	system("pause");
	return 0;
}